...
namepre = 'set5';
% different figs
Qs = {'1','Q2','Q3','Q4','Q5','Q6','Q7','Q8','Q9','Q10','tau_{A_1}','tau_{A_2}','quotient_tau_{A_1}_Q4'};
...
for i1 = 1:length(prop_sufs)
    ...
    for i2 = 1:length(Qs)
        ...
        elseif strcmp(Q_suf(1:end-6),'tau')
            xlabs{i2} = ['$\' Q_suf '$'];            
        elseif strfind(Q_suf,'tau') == 1
            Q_suf = ['tau-' num2str(tau_ind)];
            tau_ind = tau_ind + 1;
            xlabs{i2} = ['$\' Q_suf '$'];
        elseif strfind(Q_suf,'quotient')
            xlabs{i2} = ['$\' Q_suf(10:18) '/' Q_suf(end-2:end) '$'];
        end
        ...
        for j3 = 1:length(t_files)
            ...
            for i1 = 1:length(prop_sufs)
                ...
                for i2 = 1:length(Qs)
                    ...
                    elseif strfind(Q,'quotient')
                        taus = General.load_var(fullfile(proppath,'sys_half_life_amp'));
                        tau = taus{1};
                        Q_file = [x_root '_' Q(end-1:end)];
                        q1 = General.load_var(fullfile(folderpath,Q_file));
                        q = q1*tau;
                    end
...
for i1 = 1:length(prop_sufs)
    for i2 = 1:length(Qs)
        ...
        legend(hax{i1,i2},legend_subset{i1,i2},obj.legendNames2,'interpreter','latex');
